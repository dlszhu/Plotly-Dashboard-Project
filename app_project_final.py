...
def update_histogram(year_slider, user_review_slider, critics_review_slider, genres_dropdown, publishers_dropdown, sales_slider, region_radio):
    histogram_df = filter_dataframe(df, genres_dropdown, publishers_dropdown, user_review_slider, critics_review_slider, year_slider, sales_slider)
...
def update_heatmap(year_slider, user_review_slider, critics_review_slider, genres_dropdown, publishers_dropdown, sales_slider, region_radio):
    ...
    chart_df = pd.DataFrame(heatmap_df.groupby('Genre')['NA_Sales', 'JP_Sales', 'EU_Sales', 'Other_Sales'].mean())
    fig = px.imshow(chart_df.T, title='Average Regional Sales by Genre')
    return fig
...
def generate_table(year_slider, user_review_slider, critics_review_slider, genres_dropdown, publishers_dropdown, sales_slider, region_radio):
    ...
    dataframe.insert(0, column='Genre', value=dataframe.index)
    totals = ['Totals']
    totals += [dataframe[col].sum() for col in dataframe.columns[1:]]
    table = html.Table(
        # Header
        [
            html.P(
                f'Number of Games by Genre from {int(year_slider[0])} to {int(year_slider[1])}',
                className="TableTitle", style={'fontSize': 18}
            )] +
        ...
        [html.Tr([html.Th(total) for total in totals])],
        className='Table'
    )
    return table
